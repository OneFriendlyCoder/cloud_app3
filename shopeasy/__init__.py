...
def create_app(db_uri=None):
    ...
    # In Task 1.8, students will change this to /mnt/ebs-data/shopeasy.db
    # ------------------------------------------------------------------ #
    ...
    if db_uri is not None:
        ...
        use_backup = os.environ.get("APP_USE_BACKUP") == "1"
        if use_backup:
            database_path = os.environ.get("BACKUPDB_PATH", default_db)
        else:
    ...
    if database_path.startswith(("postgresql://", "postgresql+psycopg2://","postgresql+psycopg://",)):
        # PostgreSQL
        print(f"🔌 CONNECTING TO POSTGRESQL: {database_path}")
    ...
    from shopeasy.routes.cart import cart_bp
